...
Print["**********************************k=16**********************************"]
G1List = Range[1651, 1851];

three9data[[1]] = Join @@ ParallelTable[FindRequiredNoG1[0.999, i, req[[16]]],{i, start, end, step}];
three9data[[1]] = List @@ three9data[[1]];
Print[">0.999 = ", three9data[[1]]]

four9data[[1]] = Join @@ ParallelTable[FindRequiredNoG1[0.9999, i, req[[16]]],{i, start, end, step}];
...
dir = FileNameJoin[{"/home/rshiina_umass_edu/NoG1_vs_k_UW2", "NoG1_three9_vs_k_UW2_k=16"}];
If[! DirectoryQ[dir], CreateDirectory[dir]];
Do[col = Transpose[three9Alldata][[i]];
 Print["L_total = ", 45*i, ": ", col];
 Export[FileNameJoin[{dir, "NoG1_three9_vs_k_UW2_k=16_" <> ToString[45*i] <> "km.csv"}], col], {i, Length[First[three9Alldata]]}]

dir = FileNameJoin[{"/home/rshiina_umass_edu/NoG1_vs_k_UW2", "NoG1_four9_vs_k_UW2_k=16"}];
If[! DirectoryQ[dir], CreateDirectory[dir]];
Do[col = Transpose[four9Alldata][[i]];
 Print["L_total = ", 45*i, ": ", col];
 Export[FileNameJoin[{dir, "NoG1_four9_vs_k_UW2_k=16_" <> ToString[45*i] <> "km.csv"}], col], {i, Length[First[four9Alldata]]}]
