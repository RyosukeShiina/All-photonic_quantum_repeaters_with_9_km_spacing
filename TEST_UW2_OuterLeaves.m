%Abstract:
%This file tests our UW2_OutnerLeaves function.
%Run the test by executing:
%   results = runtests('TEST_UW2_OuterLeaves.m');
...
rng(1);
A0 = UW2_OuterLeaves(9, sigGKP, etas, etad, etac, k, ErrProbVec);
check_output(A0, k);
...
rng(2);
A1 = UW2_OuterLeaves(9, sigGKP, etas, etad, etac, k, ErrProbVec);
rng(2);
A2 = UW2_OuterLeaves(9, sigGKP, etas, etad, etac, k, ErrProbVec);
...
for r = 1:R
    B1 = UW2_OuterLeaves(L1, sigGKP, etas, etad, etac, k, ErrProbVec);
    B2 = UW2_OuterLeaves(L2, sigGKP, etas, etad, etac, k, ErrProbVec);
    check_output(B1, k);
    check_output(B2, k);
...
for r = 1:R
    B1 = UW2_OuterLeaves(9, sig1, etas, etad, etac, k, ErrProbVec);
    B2 = UW2_OuterLeaves(9, sig2, etas, etad, etac, k, ErrProbVec);
    check_output(B1, k);
    check_output(B2, k);
    sum1 = sum1 + sum(B1, 1);
    sum2 = sum2 + sum(B2, 1);
...
for r = 1:R
    B1 = UW2_OuterLeaves(9, sigGKP, etas, etad, etac1, k, ErrProbVec);
    B2 = UW2_OuterLeaves(9, sigGKP, etas, etad, etac2, k, ErrProbVec);
    check_output(B1, k);
    check_output(B2, k);
    sum1 = sum1 + sum(B1, 1);
    sum2 = sum2 + sum(B2, 1);
