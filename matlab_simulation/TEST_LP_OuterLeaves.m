%Abstract:
%This file tests our LP_OutnerLeaves function.
%Run the test by executing:
%   results = runtests('TEST_LP_OuterLeaves.m');
...
rng(1);
A0 = LP_OuterLeaves(9, sigGKP, etas, etad, etac, k, ErrProbVec);
check_output(A0, k);
...
rng(2);
A1 = LP_OuterLeaves(9, sigGKP, etas, etad, etac, k, ErrProbVec);
rng(2);
A2 = LP_OuterLeaves(9, sigGKP, etas, etad, etac, k, ErrProbVec);
...
for r = 1:R
    B1 = LP_OuterLeaves(L1, sigGKP, etas, etad, etac, k, ErrProbVec);
    B2 = LP_OuterLeaves(L2, sigGKP, etas, etad, etac, k, ErrProbVec);
    check_output(B1, k);
    check_output(B2, k);
...
for r = 1:R
    B1 = LP_OuterLeaves(4, sig1, etas, etad, etac, k, ErrProbVec);
    B2 = LP_OuterLeaves(4, sig2, etas, etad, etac, k, ErrProbVec);
    check_output(B1, k);
    check_output(B2, k);
    sum1 = sum1 + sum(B1, 1);
    sum2 = sum2 + sum(B2, 1);
...
for r = 1:R
    B1 = LP_OuterLeaves(4, sigGKP, etas, etad, etac1, k, ErrProbVec);
    B2 = LP_OuterLeaves(4, sigGKP, etas, etad, etac2, k, ErrProbVec);
    check_output(B1, k);
    check_output(B2, k);
    sum1 = sum1 + sum(B1, 1);
    sum2 = sum2 + sum(B2, 1);
