%Abstract:
%This file tests our R_SecretKey6State_per function.
%Run the test by executing:
%   results = runtests('TEST_SecretKey6State_per.m');
...
QerrZ = zeros(k,1);
QerrX = zeros(k,1);
out1 = R_SecretKey6State_per(QerrZ, QerrX);
assert(abs(out1 - 1) < tol);
...
out2 = R_SecretKey6State_per(QerrZ, QerrX);
assert(abs(out2 - 0) < tol);

%Test 3: Output range must be within [0,1]
k = 10;
rng(1);
QerrZ = 0.5*rand(k,1);
QerrX = 0.5*rand(k,1);
out3 = R_SecretKey6State_per(QerrZ, QerrX);
assert(out3 >= -tol);
assert(out3 <= 1 + tol);
...
out4a = R_SecretKey6State_per(QerrZ, QerrX);
out4b = R_SecretKey6State_per(QerrX, QerrZ);
assert(abs(out4a - out4b) < 1e-10);

%% Test 5: k=1 should match direct (no averaging ambiguity)
QerrZ = 0.12;
QerrX = 0.07;
out5 = R_SecretKey6State_per(QerrZ, QerrX);
